...
| `north task create <title> [--assignee A] [--labels ...] [--depends-on ...] [--body \| --body-file] [--plain \| --json]` | Create a task (lands in `drafts/`, status `ready`) |
| `north task list [--state draft\|active\|archive\|all] [--status S] [--search TEXT] [--label L] [--plain \| --json]` | List tasks (default: active). `--search` matches id/title/assignee/labels/body (case-insensitive); `--label` is exact and repeatable; `--sort id\|updated\|title\|assignee` + `--reverse` (default: id, newest first) |
