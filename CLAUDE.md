...
This is project "North", a Go program providing an **in-repo Markdown task board** (modeled on Backlog.md): a `north/` directory committed inside the user's project repo, where each task is a plain Markdown file. A task has two orthogonal axes — **state** (its folder: `drafts/`=draft, `tasks/`=active, `archive/`=archive) and **status** (a frontmatter key: ready/in_progress/done/failed/blocked — a fixed value set with free movement, any value → any other, settable in any state but only visible on the board while active). It ships a single `north` binary (CLI); AI integration is an installable skill (`north skill install`), not an MCP server. There is no daemon, no REST API, no network surface; git is the user's responsibility (North never pushes/pulls). It builds to one static binary with no runtime dependency. The only installable package is `cmd/north`; all library code lives under `internal/` (board logic in `internal/board` + `internal/tasks`, the embedded skill in `internal/skill`, the CLI in `internal/cli`).
