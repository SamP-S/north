...
from north.service import events
from north.service.notify import (
    LogTransport,
    Notifier,
    NotifyLogHandler,
    TelegramTransport,
...
def _notifier(transport, clock, monkeypatch, **kwargs) -> Notifier:
    monkeypatch.setattr("north.service.notify.time", clock)
...
def test_log_handler_forwards_warning(monkeypatch):
    ...
    handler.handle(_record("north.service.api", "merge conflict for %s", args=("x",)))
    notifier.drain()
    assert transport.sent == ["[WARNING] north.service.api: merge conflict for x"]
...
def test_log_handler_ignores_info(monkeypatch):
    ...
    notifier = _notifier(transport, FakeClock(), monkeypatch)
    handler = NotifyLogHandler(notifier)
    handler.handle(_record("north.service.api", "all fine", level=logging.INFO))
    notifier.drain()
...
def test_log_handler_dedupes_by_template(monkeypatch):
    ...
    for attempt in range(5):
        handler.handle(_record("north.x", "retry %d failed", args=(attempt,)))
    handler.handle(_record("north.x", "another problem"))
    notifier.drain()
    assert transport.sent == [
        "[WARNING] north.x: retry 0 failed",
        "[WARNING] north.x: another problem",
    ]
...
def test_log_handler_dedupe_expires(monkeypatch):
    ...
    notifier = _notifier(transport, clock, monkeypatch)
    handler = NotifyLogHandler(notifier, dedupe_window_s=3600.0)
    handler.handle(_record("north.x", "boom"))
    clock.now += 3601.0
    handler.handle(_record("north.x", "boom"))
    notifier.drain()
    assert len(transport.sent) == 2
...
def test_log_handler_excludes_notify_machinery(monkeypatch):
    ...
    notifier = _notifier(transport, FakeClock(), monkeypatch)
    handler = NotifyLogHandler(notifier)
    handler.handle(_record("north.service.notify", "send failed"))
    notifier.drain()
    assert transport.sent == []
