...
def test_show_renders(ns, make_ctx, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    rc = feature.show(ns(project="p", feature="feat-1"), make_ctx(board=handler))
...
def test_status_sets(ns, make_ctx, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    rc = feature.status(
        ns(project="p", feature="f", status="in_progress"), make_ctx(board=handler)
    )
    assert rc == 0
...
def test_delete_declined(
    ...
    rc = feature.delete(
        ...
        make_ctx(board=lambda r: calls.append(r.method) or httpx.Response(200, json={})),
    )
    assert rc == 0
...
def test_requeue(ns, make_ctx, capsys: pytest.CaptureFixture[str]) -> None:
    rc = feature.requeue(
        ...
        make_ctx(board=lambda r: httpx.Response(200, json={"requeued": 3})),
    )
    assert rc == 0
...
def test_promote(ns, make_ctx, capsys: pytest.CaptureFixture[str]) -> None:
    rc = feature.promote(ns(project="p", feature="f"), make_ctx(board=_dummy))
    assert rc == 0
...
def test_list_review(ns, make_ctx, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    rc = feature.list_(
        ns(review=True, archived=False, project=None), make_ctx(board=handler)
    )
    out = capsys.readouterr().out
    assert rc == 0
...
def test_list_project_scoped(ns, make_ctx, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    rc = feature.list_(
        ns(review=False, archived=False, project="p"), make_ctx(board=handler)
    )
    assert rc == 0
...
def test_list_all(ns, make_ctx, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    rc = feature.list_(
        ns(review=False, archived=False, project=None), make_ctx(board=handler)
    )
    assert rc == 0
...
def test_list_archived_include(ns, make_ctx) -> None:
    ...
    rc = feature.list_(
        ns(review=False, archived=True, project="p"), make_ctx(board=handler)
    )
    assert rc == 0
...
def test_list_review_and_archived_mutually_exclusive(ns, make_ctx) -> None:
    with pytest.raises(CLIError, match="mutually exclusive"):
        feature.list_(
            ns(review=True, archived=True, project=None), make_ctx(board=_dummy)
        )
...
def test_list_archived_requires_project(ns, make_ctx) -> None:
    with pytest.raises(CLIError, match="requires --project"):
        feature.list_(
            ns(review=False, archived=True, project=None), make_ctx(board=_dummy)
        )
