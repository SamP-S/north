...
feature_review (emitted when the north/brief note lands on a review feature)."""
...
from north.service.board.parser import parse_conversation, parse_feature, parse_task
from north.service.models import BoardState, ProjectModel
from north.service.notify import Notifier
...
def test_brief_note_on_non_review_feature_does_not_emit(
    ...
    response = client.post(
        "/api/projects/demo/features/feat-x/comments",
        json={"kind": "note", "author": "north/brief", "text": "early note"},
    )
    assert response.status_code == 200
    _drain()
    assert transport.sent == []
