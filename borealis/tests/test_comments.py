...
from borealis.service.board.parser import parse_feature, parse_task, parse_thread
from borealis.service.board.writer import append_thread_entry
from borealis.service.models import BoardState, ProjectModel, ThreadEntryKind
...
def test_append_and_parse_round_trip(tmp_path: Path) -> None:
    ...
    append_thread_entry(path, "question", "aurora/implement", AT, "Is the API versioned?")
    ...
    assert entries[0].author == "aurora/implement"
...
def test_task_comments_round_trip(
    ...
    resp = client.post(
        base, json={"kind": "question", "author": "aurora/implement", "text": "Versioned?"}
    )
    ...
    assert comments[0]["author"] == "aurora/implement"
...
def _block_on_question(api: tuple[TestClient, BoardState]) -> None:
    ...
    from borealis.service.models import BlockedReason, TaskStatus
