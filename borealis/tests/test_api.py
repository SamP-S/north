"""Tests for the Borealis API endpoints."""
...
import pytest
from borealis.service.api.deps import get_board_context, set_board_context
...
def test_patch_task_status(api: tuple[TestClient, BoardState], board_repo: Path) -> None:
    ...
    with patch("borealis.service.api.tasks.commit_and_push_board"):
        resp = client.patch(
...
def test_patch_blocked_stamps_reason_and_requeue_clears_it(
    ...
    from borealis.service.models import BlockedReason
...
def test_patch_to_ready_clears_ready_at(
    ...
    from borealis.service.board.parser import parse_task
    from borealis.service.board.writer import update_task_frontmatter
...
def test_patch_feature_status(api: tuple[TestClient, BoardState], board_repo: Path) -> None:
    ...
    with patch("borealis.service.api.features.commit_and_push_board"):
...
def test_delete_task_removes_file(api: tuple[TestClient, BoardState], board_repo: Path) -> None:
    ...
    with patch("borealis.service.api.tasks.commit_and_push_board") as push:
...
def queue_api(board_repo: Path) -> tuple[TestClient, BoardState]:
    from borealis.service import main as main_module
...
def test_delete_feature_draft_only(
    ...
    with patch("borealis.service.api.features.commit_and_push_board") as push:
...
def test_derive_project_name_strips_git_suffix() -> None:
    from borealis.service.main import derive_project_name
