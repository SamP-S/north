...
:class:`NorthContext` whose Aurora/Borealis clients are constructed lazily, so a
single-service command never depends on the other service being reachable. All
...
def _add_status(sub: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    """Wire ``north status`` (combined) and ``status aurora|borealis``."""
    p_status = sub.add_parser("status", help="show combined service status")
    p_status.set_defaults(func=status.status)
    status_sub = p_status.add_subparsers(dest="status_target", metavar="<target>")

    ps_aurora = status_sub.add_parser("aurora", help="aurora runner status only")
    ps_aurora.set_defaults(func=status.aurora)

    ps_borealis = status_sub.add_parser("borealis", help="borealis board status only")
    ps_borealis.set_defaults(func=status.borealis)


def _add_observe_control(sub: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    """Wire ``north logs`` and ``north pause`` / ``resume``."""
    p_logs = sub.add_parser("logs", help="stream agent output events")
    _project_opt(p_logs)
    p_logs.set_defaults(func=observe.logs)

    p_pause = sub.add_parser("pause", help="pause the runner")
    p_pause.set_defaults(func=control.pause)

    p_resume = sub.add_parser("resume", help="resume the runner")
    p_resume.set_defaults(func=control.resume)
...
def _add_service_actions(svc_unit: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    ...
    Shared by both the ``service aurora`` and ``service borealis`` parsers; the
    ``unit`` default is set on the parent unit parser and propagates here.
    """
...
def _add_service(sub: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    """Wire ``north service aurora|borealis <action>`` and ``service status``."""
    p_service = sub.add_parser("service", help="manage the systemd services")
    p_service.set_defaults(func=_group_help(p_service))
    service_sub = p_service.add_subparsers(dest="service_target", metavar="<target>")

    p_aurora = service_sub.add_parser("aurora", help="manage the aurora service")
    p_aurora.set_defaults(func=_group_help(p_aurora), unit="aurora")
    _add_service_actions(p_aurora.add_subparsers(dest="action", metavar="<action>"))

    p_borealis = service_sub.add_parser("borealis", help="manage the borealis service")
    p_borealis.set_defaults(func=_group_help(p_borealis), unit="borealis")
    _add_service_actions(p_borealis.add_subparsers(dest="action", metavar="<action>"))

    p_status = service_sub.add_parser("status", help="aggregate status of both units")
    p_status.set_defaults(func=service.aggregate_status)
...
def _add_feature(sub: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    """Wire ``north feature`` (Borealis CRUD + Aurora review verbs + list)."""
    ...
    f_approve = feature_sub.add_parser("approve", help="approve and merge a feature")
    f_approve.add_argument("project")
    f_approve.add_argument("feature")
    f_approve.set_defaults(func=review.approve)

    f_rollback = feature_sub.add_parser("rollback", help="roll back a merged feature")
    f_rollback.add_argument("project")
    f_rollback.add_argument("feature")
    f_rollback.set_defaults(func=review.rollback)

    f_reject = feature_sub.add_parser("reject", help="reject and discard a feature")
    f_reject.add_argument("project")
    f_reject.add_argument("feature")
    f_reject.add_argument("-y", "--yes", action="store_true", help="skip confirmation prompt")
    f_reject.set_defaults(func=review.reject)
