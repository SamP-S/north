...
def list_(_args: argparse.Namespace, ctx: NorthContext) -> int:
    ...
    items: list[dict[str, Any]] = ctx.borealis.get("/api/projects")
    if not items:
...
def show(args: argparse.Namespace, ctx: NorthContext) -> int:
    ...
    data: dict[str, Any] = ctx.borealis.get(f"/api/projects/{args.project}")
...
def register(args: argparse.Namespace, ctx: NorthContext) -> int:
    ...
    if args.name:
        body["name"] = args.name
    data: dict[str, Any] = ctx.borealis.post("/api/projects/register", body=body)
    name = (
        data.get("name")
        or args.name
...
def update(args: argparse.Namespace, ctx: NorthContext) -> int:
    ...
    data: dict[str, Any] = ctx.borealis.patch(f"/api/projects/{args.project}", body=body)
...
def unregister(args: argparse.Namespace, ctx: NorthContext) -> int:
    ...
    active: list[Any] = []
    try:
        features: list[dict[str, Any]] = ctx.borealis.get(
    ...
    ctx.borealis.delete(f"/api/projects/{project}")
