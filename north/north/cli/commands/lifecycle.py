...
kills the process, whereas ``north pause`` keeps it running but idles the
supervisor. ``service status`` reports OS state only (unit + boot/linger); app
health lives behind the top-level ``north status`` command.

Handlers are unit-parametrized: each reads ``args.unit`` (``"aurora"`` or
``"borealis"``), set via ``set_defaults(unit=...)`` on the per-unit subparser.
"""
