...
def test_list_empty(ns, make_ctx, capsys: pytest.CaptureFixture[str]) -> None:
    rc = conversation.list_(
        ns(project="p"), make_ctx(borealis=lambda r: httpx.Response(200, json=[]))
    )
    assert rc == 0
...
def test_list_pending_cross_project(
    ...
    rc = conversation.list_(ns(project=None, pending=True), make_ctx(borealis=handler))
...
def test_show_renders(ns, make_ctx, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    rc = conversation.show(
        ns(project="p", conversation_id="c-1"), make_ctx(borealis=handler)
    )
    out = capsys.readouterr().out
    assert rc == 0
