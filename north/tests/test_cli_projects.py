...
def test_list_empty(ns, make_ctx, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    rc = projects.list_(ns(), make_ctx(borealis=handler))
    assert rc == 0
...
def test_show_renders_fields(
    ...
    rc = projects.show(ns(project="app"), make_ctx(borealis=handler))
    out = capsys.readouterr().out
    assert rc == 0
...
def test_unregister_declined_skips_delete(
    ...
    rc = projects.unregister(ns(project="app", yes=False), make_ctx(borealis=handler))
    out = capsys.readouterr().out
    assert rc == 0
...
def test_update_auto_merge_flag(ns, make_ctx) -> None:
    ...
    rc = projects.update(
        ns(project="app", base_branch=None, auto_merge=True), make_ctx(borealis=handler)
    )
    assert rc == 0
...
def test_unregister_yes_skips_prompt(ns, make_ctx) -> None:
    ...
    rc = projects.unregister(ns(project="app", yes=True), make_ctx(borealis=handler))
    assert rc == 0
    assert "DELETE" in calls
